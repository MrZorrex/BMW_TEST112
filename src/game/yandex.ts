...
/**
 * Запасной абсолютный адрес SDK. НУЖЕН ТОЛЬКО при интеграции через свой домен
 * (iframe) — дока: yandex.ru/dev/games/doc/ru/sdk/sdk-about#iframe
 *
 * В релизном архиве для Яндекс Игр этой строки быть НЕ ДОЛЖНО. Причина в
 * домене: `*.s3.yandex.net` — адрес внутреннего объектного хранилища сервиса,
 * и автоматическая проверка Консоли отклоняет релиз с замечанием
 * «Обнаружена ссылка на сервисное хранилище / Файл содержит URL-адрес
 * внутреннего хранилища сервиса». На сервере Яндекса SDK и так отдаётся по
 * относительному пути тегом `<script src="/sdk.js">` в index.html.
 *
 * Поэтому адрес пуст по умолчанию и подставляется только из переменной сборки:
 *   VITE_YANDEX_SDK_URL="https://sdk.games.s3.yandex.net/sdk.js" npm run build
 * (так собирается отдельный вариант для своего домена — в Консоль он не грузится).
 */
const SDK_ABS_URL: string = import.meta.env.VITE_YANDEX_SDK_URL ?? "";
 ...
 *
 * `true` только когда задан запасной адрес SDK (сборка для своего домена/iframe)
 * И страница загружена по http(s). В релизе для Яндекс Игр адрес пуст — значит,
 * единственно верный источник SDK это тег `/sdk.js`, который раздаёт сам сервер
 * платформы, и догружать нечего. В dev-режиме и при открытии файла двойным
 * кликом (file://) догружать тоже нечего: на официальном локальном запуске
 * Яндекс Игр свой сервер проксирует /sdk.js, и глобал `YaGames` появляется ещё
 * до этого места.
...
function shouldLoadSdkDynamically(): boolean {
  if (!SDK_ABS_URL) return false; // релиз для Яндекс Игр: SDK берётся только из /sdk.js
